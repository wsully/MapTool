...
public class EndUserGUI extends JPanel implements ActionListener{
    ...
    //Test map edges
    Edge a = new Edge (one, two, 2*50);
    Edge b = new Edge (two, three, 1*50);
    Edge c = new Edge (two, four, 6*50);
    Edge d = new Edge (four, five, 1*50);
    Edge e = new Edge (four, six, 6*50);
    Edge f = new Edge (six, seven, 1*50);
    Edge gee = new Edge (six, eight, 4*50);
    Edge h = new Edge (eight, nine, 1*50);
    Edge i = new Edge (eight, ten, 7*50);
    Edge j = new Edge (ten, eleven, 1*50);
    Edge k = new Edge (ten, twelve, 2*50);
    Edge l = new Edge (twelve, thirteen, 8*50);
    Edge m = new Edge (twelve, fourteen, 10*50);
    Edge n = new Edge (fourteen, fifteen, 2*50);
    Edge o = new Edge (fifteen, sixteen, 1*50);
    Edge p = new Edge (fifteen, seventeen, 7*50);
    Edge q = new Edge (seventeen, eighteen, 1*50);
    Edge r = new Edge (seventeen, nineteen, 10*50);
    Edge s = new Edge (nineteen, twenty, 6*50);
    Edge t = new Edge (twenty, twentyone, 1*50);
    Edge u = new Edge (nineteen, twentytwo, 4*50);
    Edge v = new Edge (twentytwo, twentythree, 2*50);
    Edge w = new Edge (twentytwo, four, 6*50);
	Node[] testNodes = new Node[]{one, two, three, four, five, six, seven, eight, nine, ten, eleven, twelve,
...
//		endBuildingSEL.setVisible(true);
		...
		searchButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				...
				startNode = testNodes[startInt];
				endNode = testNodes[endInt];
...
//				}
	...
	public class MyGraphics extends JComponent implements MouseMotionListener{
		...
		public void paintComponent(Graphics g) {
			...
			repaint();
			revalidate();
			g.drawImage(mapImage, 0, (750-mapImage.getHeight(this))/4, this);
